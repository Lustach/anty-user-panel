    ...
    <v-card class="rounded-xl">
      ...
      <v-card-text>
        <v-row>
          ...
          >
            <select
              v-model="selectedPlan.additionalProfiles"
              class="custom-input custom-input-select"
            >
              <option
                v-for="item in additionalProfiles"
                :key="`dolphin-anty-user-panel-plan-dialog-${item.value}`"
          ...
          >
            ...
            >
              ...
              >
                ...
                <span>{{ $t('plans.oneUserIncluded') }}</span>
              </v-tooltip>
            </div>
          </v-col>
          <v-col
          ...
          >
            <input
              v-model="selectedPlan.additionalUsers"
              :placeholder="$t('plans.additionalUsers')"
              spellcheck="false"
              style="max-width: 60px; margin-left: auto;"
              type="number"
              :min="profile.subscription.users.limit - 1"
              class="custom-input"
              @change="changeAdditionalUsers"
            >
          </v-col>
          ...
          >
            ...
            >
              <option
                v-for="prolongMonth in prolongMonthsFiltered"
          ...
          <v-col
            ...
            class="text-end"
          >
            <span id="discountAmount">{{ this.balance.new.discountPercentageRate }}%</span>
          </v-col>
          <!-- ДИСКОНТ КОНЕЦ -->
          ...
          >
            {{
              balance.new ? parseFloat(balance.new.difference + estimateDetailsPaymentSystemFee).toLocaleString('en-US', {
                style: 'currency',
                currency: 'USD',
          ...
          <!-- ОПИСАНИЕ СУММЫ -->
          <v-col
            v-if="showFullEstimate && showPayments"
            cols="12"
          >
            <v-row>
              ...
              <v-col
                ...
                class="text-end py-1"
              >
                {{ parseFloat(balance.new.basePrice).toLocaleString('en-US', {style: 'currency', currency: 'USD', maximumFractionDigits: 2, minimumFractionDigits: 2}) }}
              </v-col>
              ...
              <v-col
                v-if="selectedPlan.plan !== 'free' && balance.new.additionalUsersPrice > 0"
                cols="8"
                class="py-1"
              >
              ...
              <v-col
                ...
                class="text-end py-1"
              >
                {{ parseFloat(balance.new.additionalUsersPrice).toLocaleString('en-US', {style: 'currency', currency: 'USD', maximumFractionDigits: 2, minimumFractionDigits: 2}) }}
              </v-col>
              ...
              <v-col
                ...
                class="text-end py-1"
              >
                {{ parseFloat(balance.new.additionalProfilesPrice).toLocaleString('en-US', {style: 'currency', currency: 'USD', maximumFractionDigits: 2, minimumFractionDigits: 2}) }}
              </v-col>
              ...
              <!-- СКИДКА -->
              <v-col
                v-if="this.balance.new.discount < 0"
                cols="8"
                class="py-1"
              >
                {{ $t('plans.discount') }}
              ...
              <v-col
                ...
                class="text-end py-1"
              >
                {{ parseFloat(this.balance.new.discount).toLocaleString('en-US', {style: 'currency', currency: 'USD', maximumFractionDigits: 2, minimumFractionDigits: 2}) }}
              </v-col>
              <!-- СКИДКА КОНЕЦ -->
              ...
              <v-col
                ...
                class="py-1"
              >
                {{ $t('plans.amountDescription.balance') }}
              </v-col>
              <v-col
                v-if="balance.balance"
                cols="4"
                class="text-end py-1"
              >
                -{{ parseFloat(balance.balance).toLocaleString('en-US', {style: 'currency', currency: 'USD', maximumFractionDigits: 2, minimumFractionDigits: 2}) }}
              </v-col>
            </v-row>
          </v-col>
          ...
          >
            ...
            <v-btn
              ...
              rounded
              class="mb-2"
              :loading="payNowLoading || loadingBalance"
              :disabled="selectedPlan.plan !== 'free' && parseInt(estimate, 10) === 0 "
              style="width: 150px"
              @click="tryToPay"
            >
              {{ showPayments ? $t('common.payAndGo') : $t('common.upgrade') }}
            </v-btn>
            <DialogManualPaymentInfo
              v-if="selectedPlan.paymentMethod==='capitalist'"
              :sum="balance.new ? parseFloat(balance.new.difference + estimateDetailsPaymentSystemFee).toLocaleString('en-US', {
                style: 'currency',
                currency: 'USD',
                maximumFractionDigits: 2,
                minimumFractionDigits: 2
            ...
            <!--            <div-->
            <!--              v-if="selectedPlan.paymentMethod == 'capitalist'"-->
            <!--              class="capitalist-info"-->
            <!--              v-html="$t('plans.capitalistInfo')"-->
            <!--            />-->
            <!--            <div-->
            <!--              v-if="selectedPlan.paymentMethod == 'usdt'"-->
            <!--              class="capitalist-info"-->
            <!--              v-html="$t('plans.usdtInfo')"-->
            <!--            />-->

            <!-- ВАРНИНГИ АВТОМАТИЧЕСКОГО ПРОДЛЕНИЯ ПРИ ОПЛАТЕ КАРТОЙ -->
            <div
              v-if="showPayments && selectedPlan.paymentMethod === 'card'"
            >
              <v-icon :size="12">
                fas fa-check
              </v-icon>
...
export default {
  ...
  computed: {
    ...
    ...mapFields('plans', ['selectedPlan', 'balance','needPay']),
    ...mapFields('main', ['successfulPaymentAlert','successfulUpgradeAlert']),

    estimateDetailsPaymentSystemFee(){
      //  this.balance.new.difference
      const paymentMethod = this.selectedPlan.paymentMethod;
      let fee = 0
      switch (paymentMethod) {
      case 'card':
        fee = this.balance.new.difference * 0.03
        break;
      case 'crypto':
        fee = this.balance.new.difference * 0.05
        break;
    ...
    subscriptionRenewDate() {
      return this.moment().add(parseInt(this.balance.new.days, 10), 'days').locale(this.$i18n.locale).format("Do MMM YYYY")
    },
    prolongMonthsFiltered() {
      if (!this.prolongMonthsAppend || this.profile.subscription.plan === 'free') {
        return this.prolongMonths
      } else {
        return this.prolongMonths.filter((el) => {
          if (typeof el.days != 'undefined') {
            return true
          }
          if (this.prolongMonthsAppend.length < 1) {
            return true
          }

          if(!(this.profile.subscription.plan === 'free' &&  this.profile.subscription.browserProfiles.limit === 10)) {
            return   this.prolongMonthsAppend[0].days < (el.value * 30)
          }
    ...
    prolongMonths() {
      const prolongMonths = [...this.prolongMonthsAppend]
    ...
    showPayments() {
      if (typeof this.balance.new === "undefined" ) {
        return false
      }
      return parseFloat(this.balance.new.difference) > 0
    },
  },
  ...
  watch: {
    ...
    prolongMonthsFiltered(prolongMonthsFiltered){
      let find = prolongMonthsFiltered.find(el => {
        return  el.value === this.selectedPlan.prolongMonths
      })
      if(find == undefined){
    ...
    'selectedPlan.additionalUsers': {
      handler() {
        this.needUpdateDaysLabel = true
      }
    },
    'selectedPlan.additionalProfiles': {
      handler() {
        this.needUpdateDaysLabel = true
      }
    },
    ...
    'balance': {
      handler() {
        if (this.needUpdateDaysLabel) {
          this.updateProlongMonthsAppend()
          this.needUpdateDaysLabel = false


          /* if( this.profile.subscription.plan === 'free' &&  this.profile.subscription.browserProfiles.limit === 10){
            this.selectedPlan.prolongMonths = val.new.days / 30
          }*/
        }
      }
    },
  ...
  created() {
    ...
    this.selectedPlan.additionalUsers = this.profile.subscription.users.limit - 1
    this.simpleEstimateCalculation();
    this.getBalanceByPlan = debounce(this.getBalanceByPlan, 300)
  ...
  methods: {
    ...
    updateProlongMonthsAppend() {
      if (this.balance && !(this.profile.subscription.plan === 'free' &&  this.profile.subscription.browserProfiles.limit === 10)) {
        this.prolongMonthsAppend = [{
          value: 0,
          text: this.$t('plans.availableToYou', [this.balance.new.days]),
          days: this.balance.new.days
        }];
      }
      if(this.profile.subscription.plan === 'free' &&  this.profile.subscription.browserProfiles.limit === 10){
        this.selectedPlan.prolongMonths =  this.balance.new.days / 30
      }
      this.needUpdateDaysLabel = false
    },
    async getBalanceByPlan() {
    ...
    showSuccessfulAlert(){
      if(this.showPayments){
        ...
        this.successfulUpgradeAlert = false
      }
    ...
    async  tryToPay(){
      ...
      else if(this.selectedPlan.paymentMethod==='usdt'){
        this.$store.dispatch('main/openDialog', 'manualPaymentInfoDialogUSDT')
      }
      else if(  this.selectedPlan.plan === 'free' && this.balance.balance > 0 && this.balance.new.totalPrice === 0){
        this.$store.dispatch('main/openDialog', 'unsubscribePlan')
      } else {
        await this.pay()
      }
    },
    async pay() {
      ...
      try {
        ...
        this.$set(this.error, 'show', false)
        this.$set(this.error, 'message', '')
        const response = await this.$store.dispatch('plans/createUpdateInvoice')
        ...
        if (
          ...
          this.$amplitude.getInstance().logEvent('Payment Invoice Create', {
            plan: this.profile.subscription.plan,
            'payment gateway': this.selectedPlan.paymentMethod,
          })
