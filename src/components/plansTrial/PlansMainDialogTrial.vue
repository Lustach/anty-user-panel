      ...
      <v-card-text>
        <v-row>
          ...
          >
            ...
            <v-btn
              v-if="!['usdt','capitalist'].includes( selectedPlan.paymentMethod) "
              color="primary"
              rounded
              class="mb-2"
...
export default {
  name: 'PlansmainDialogTrialTrial',
  ...
  computed: {
    ...
    subscriptionRenewDate() {
      ...
      return this.moment().add(parseInt(prolongDays, 10), 'days').locale(this.$i18n.locale).format("Do MMM YYYY")
    },
  ...
  methods: {
    closeDialog() {
      this.$store.dispatch('plans/closeDialog', 'mainDialogTrial')
    },
